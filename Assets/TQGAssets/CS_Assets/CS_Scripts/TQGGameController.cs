...
namespace TriviaQuizGame
{
    ...
    public class TQGGameController : MonoBehaviour
    {
        ...
        internal float playTime = 0;


        [Header("Game Events")]
        public GameEvent victorySinglePlayerEvent;
        public GameEvent victoryMultiplayerEvent;
        ...
        void Update()
        {
            ...
            if (currentPlayer < players.Length)
            {
                ...
                if (players[currentPlayer].score.Value < players[currentPlayer].scoreCount)
                {
                    // Count up to the courrent value
                    players[currentPlayer].score.Value = Mathf.Lerp(players[currentPlayer].score.Value, players[currentPlayer].scoreCount, Time.deltaTime * 10);

                    // Round up the score value
                    players[currentPlayer].score.Value = Mathf.CeilToInt(players[currentPlayer].score.Value);
        ...
        public void ShowResult(bool isCorrectAnswer)
        {
            ...
            if (questions[currentQuestion].followup != null && (questions[currentQuestion].followup != String.Empty || questions[currentQuestion].followupImage || questions[currentQuestion].followupSound))
            {
                ...
                if (questions[currentQuestion].followupImage)
                {
                    // Unhide the image object
                    questionObject.Find("Image").gameObject.SetActive(true);
                ...
                if (questionObject.Find("ButtonPlaySound"))
                {
                    if (questions[currentQuestion].followupSound != null)
                    {
                        // Activate the sound button, since it may have been hidden before
                        questionObject.Find("ButtonPlaySound").gameObject.SetActive(true);
        ...
        void UpdateScore()
        {
            ...
            if (scoreToVictory > 0 && players[currentPlayer].score.Value >= scoreToVictory) StartCoroutine(Victory(0));
        }
        ...
        IEnumerator GameOver(float delay)
        {
            ...
            //playTime = DateTime.Now - playTime;
            ...
            if (gameOverCanvas)
            {
                ...
                if (players[currentPlayer].score.Value > highScore)
                {
                    highScore = players[currentPlayer].score.Value;

                    //Register the new high score
#if UNITY_5_3 || UNITY_5_3_OR_NEWER
					PlayerPrefs.SetFloat(SceneManager.GetActiveScene().name + "HighScore", players[currentPlayer].score.Value);
#else
        ...
        IEnumerator Victory(float delay)
        {
            ...
            if (numberOfPlayers == 1 && victorySinglePlayerEvent != null)
                victorySinglePlayerEvent.Raise();
            else if (victoryMultiplayerEvent != null)
                victoryMultiplayerEvent.Raise();
            ...
            if (victoryCanvas)
            {
                ...
                if (victoryCanvas.Find("ScoreTexts/Players"))
                {
                    ...
                    Array.Sort(players, delegate (Player x, Player y) { return y.score.Value.CompareTo(x.score.Value); });
                    ...
                    for (index = 0; index < numberOfPlayers; index++)
                    {
                        if (players[index].score.Value <= 0 || index >= winnerCount) victoryCanvas.Find("ScoreTexts/Players").GetChild(index).Find("WinnerIcon").gameObject.SetActive(false);
                    }
                    ...
                    for (index = 0; index < numberOfPlayers; index++)
                    {
                        ...
                        victoryCanvas.Find("ScoreTexts/Players").GetChild(index).GetComponent<Text>().text = players[index].name.Value;
            ...
            for (index = 0; index < players.Length; index++)
            {
                if (playersObject)
                {
                    if (index < numberOfPlayers)
                    {
                        ...
                        if(players[index].score != null)
                            players[index].score.Value = 0;
                        players[index].scoreText.GetComponent<Text>().text = players[index].score.ToString();
            ...
            foreach (XmlNode XmlRecord in xmlRecords)
            {
                ...
                foreach (XmlNode XmlQuestion in XmlQuestions)
                {
                    ...
                    if (XmlQuestion.Name == "Followup")
                    {
                        ...
                        if (XmlQuestion.Attributes.GetNamedItem("image") != null) questionsTemp[questionIndex].followupImage = Resources.Load<Sprite>("Images/" + XmlQuestion.Attributes.GetNamedItem("image").InnerText);
                        ...
                        if (XmlQuestion.Attributes.GetNamedItem("sound") != null) questionsTemp[questionIndex].followupSound = Resources.Load<AudioClip>("Sounds/" + XmlQuestion.Attributes.GetNamedItem("sound").InnerText);
                    }
