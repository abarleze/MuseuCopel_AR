	...
	public class TQGCategoryGrid:MonoBehaviour
	{
		...
		void Start()
		{
			...
			if ( categoryGridObject ) 
			{
                ...
                float gridHeight = (Mathf.CeilToInt(1.0f * categories.Length / categoryGridObject.GetComponent<GridLayoutGroup>().constraintCount)) * (categoryGridObject.GetComponent<GridLayoutGroup>().cellSize.y + categoryGridObject.GetComponent<GridLayoutGroup>().spacing.y);
		...
		public void SetCategory( int setValue )
		{
			...
			if ( categoryGridObject ) 
			{
				...
				else
				{
                    ...
                    if ( categories[setValue].subcategories.Length > 0 )    categories[setValue].UpdateSubcategories();
