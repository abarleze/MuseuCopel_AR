...
namespace TriviaQuizGame
{
	...
	public class TQGCategorySelector:MonoBehaviour
	{
		...
		public void SetCategory( int setValue )
		{
            ...
            if (categories[setValue].subcategories != null && categories[setValue].subcategories.Length > 0) categories[setValue].UpdateSubcategories();
		...
		{
			// Check if we have a Dynamic XML component attached to the category
			if ( categories[currentCategory].GetComponent<TQGDynamicXML>() ) 
			{
				TQGDynamicXML newDynamicXML = Instantiate(categories[currentCategory].GetComponent<TQGDynamicXML>()) as TQGDynamicXML;

				gameController.GetComponent<TQGGameController>().dynamicXML = newDynamicXML;
			}
